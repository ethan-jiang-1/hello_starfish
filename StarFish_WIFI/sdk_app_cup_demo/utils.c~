...
char* strdup(const char *str)
{
    ...
    if ((p = qcom_mem_alloc(A_STRLEN(str) + 1)) != NULL) 
        return (A_STRCPY(p, str));
...
size_t strnlen(const char * pSrc, size_t _MaxCount)
{
    size_t len = 0;
...
static noinline char *put_dec(char *buf, long long num)
{
	while (1) {
		unsigned rem;
...
int vasprintf(char **strp, const char *fmt, va_list args) 
{
    ...
    if ((*strp = qcom_mem_alloc(size + 1)) == NULL) {
        return -1;
    }
