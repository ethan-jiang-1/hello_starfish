...
#define IMG_HEADER_LEN      32//%B1Q,300,1234,1234,1,1234567890,
                       ...
                       A_INT32 partition_index,A_INT32 test_flag)
{
	...
	A_CHAR WifiImgHeader[IMG_HEADER_LEN]= { 0 };
	
	char * myresult=NULL;
	unsigned long testval=1;
...
retry:
	...
	A_PRINTF("step 2.1\r\n");
	memset(WifiImgHeader,0,IMG_HEADER_LEN);
        if (0 == fd_act) {
...
if(test_flag==1)
{
	...
	testval+=100;
	myresult=simple_itoa(testval);
	A_PRINTF("myresult:%s\r\n",myresult);
	memcpy(WifiImgHeader,"%B1Q,300,1234,1234,1,1234567890,",32);
	A_PRINTF("total:%d,current:%d\r\n", total_rx_bytes ,data_blk_size);
/*
...
*/
/*send cloud data to uart*/
			/*first send header*/

			Uart_Write_Block(uart_mcu_com, (A_CHAR *)WifiImgHeader,IMG_HEADER_LEN);

			/*second send data*/
			Uart_Write_Block(uart_mcu_com, (A_CHAR *)data_blk_buf,data_blk_size);


/*Get reply from uart*/

//	Uart_Read_Block(uart_mcu_com,uartRxBuf,readlen);

       
/*end get reply from kl26*/
