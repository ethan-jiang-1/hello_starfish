...
#define free_z(p) do { if (p) { qcom_mem_free(p); (p) = 0; } } while (0)
...
/*
 * Dummy definitions to allow compilation on qcom_sdk
 */
...
static size_t _write(int fd, const void *buf, size_t count)
{
    size_t written   = 0;
    size_t thisTime     = 0;
    ...
    while (count != written) {
        ...
        thisTime = qcom_send(fd, (char *)buf + written, count - written, 0);
...
static size_t _write_string(int fd, const void *buf)
{
    return _write(fd, buf, A_STRLEN(buf));
}
...
char* cli_command_name(struct cli_def *cli, struct cli_command *command)
{
    ...
    while (command) {
        ...
        if (asprintf(&name, "%s%s%s", command->command, *o ? " " : "", o) == -1) {
            A_PRINTF("Couldn't allocate memory for command_name: %s", command->command);
            free_z(o);
            return NULL;
        }
...
{
    struct unp *u, *n;
    if (!(n = qcom_mem_alloc(sizeof(struct unp)))) {
        A_PRINTF("Couldn't allocate memory for unp struct");
        return;
    }

    if (!(n->username = strdup(username))) {
        A_PRINTF("Couldn't allocate memory for username: %s", username);
        free_z(n);
        return;
    }

    if (!(n->password = strdup(password))) {
        A_PRINTF("Couldn't allocate memory for password: %s", password);
        free_z(n->username);
        free_z(n);
        return;
    }
...
void cli_allow_enable(struct cli_def *cli, const char *password)
{
    ...
    if (!(cli->enable_password = strdup(password))) {
        A_PRINTF("Couldn't allocate memory for enable password: %s", password);
    }
}
...
static int cli_build_shortest(struct cli_def *cli, struct cli_command *commands)
{
    ...
    for (c = commands; c; c = c->next) {
        ...
        c->unique_len = A_STRLEN(c->command);
                                         ...
                                         const char *help)
{
    ...
    if (!(c = qcom_mem_calloc(sizeof(struct cli_command), 1))) 
        return NULL;
...
int cli_int_exit(struct cli_def *cli, const char *command, char *argv[], int argc)
{
    ...
    if (cli->mode > MODE_CONFIG) cli_set_configmode(cli, MODE_CONFIG, NULL);
    else cli_set_configmode(cli, MODE_EXEC, NULL);
...
struct cli_def* cli_init()
{
    ...
    if (!(cli = qcom_mem_calloc(sizeof(struct cli_def), 1))) return 0;

    cli->buf_size = 1024;

    if (!(cli->buffer = qcom_mem_calloc(cli->buf_size, 1))) {
        free_z(cli);
        return 0;
    }
    ...
    while (nwords < max_words - 1) {
        if (!*p || *p == inquote || (word_start && !inquote && (isspace((unsigned char)(*p)) || *p == '|'))) {
            if (word_start) {
                ...
                memcpy(words[nwords] = qcom_mem_alloc(len + 1), word_start, len);
                words[nwords++][len] = 0;
            }
            ...
            if (!word_start) {
                ...
                } else if (!isspace((unsigned char)(*p))) word_start = p;
            }
                            ...
                            int start_word, int filters[])
{
    ...
    if (words[start_word][A_STRLEN(words[start_word]) - 1] == '?') {
        int l = A_STRLEN(words[start_word]) - 1;
    ...
    for (c = commands; c; c = c->next) {
        ...
        if (strncasecmp(c->command, words[start_word], A_STRLEN(words[start_word]))) 
            continue;
    ...
    AGAIN:
        ...
        CORRECT_CHECKS:
            for (f = 0; rc == CLI_OK && filters[f]; f++) {
                ...
                argc = n - (filters[f] + 1);
                len = A_STRLEN(argv[0]);
                if (argv[argc - 1][A_STRLEN(argv[argc - 1]) - 1] == '?') {
                    if (argc == 1) {
                        int i;
                ...
                *filt = qcom_mem_calloc(sizeof(struct cli_filter), 1);
...
static int cli_get_completions(struct cli_def *cli, const char *command, char **completions, int max_completions)
{
    ...
    if (!command[0] || command[A_STRLEN(command) - 1] == ' ') 
        num_words++;
    ...
    if (filter) { // complete filters
        ...
        if (filter == num_words - 1) len = A_STRLEN(words[num_words - 1]);

        for (i = 0; filter_cmds[i].cmd && k < max_completions; i++) {
            if (!len || (len < A_STRLEN(filter_cmds[i].cmd) && !strncmp(filter_cmds[i].cmd, words[num_words - 1], len))) 
    ...
    for (c = cli->commands, i = 0; c && i < num_words && k < max_completions; c = n) {
        ...
        if (words[i] && strncasecmp(c->command, words[i], A_STRLEN(words[i]))) 
            continue;

        if (i < num_words - 1) {

            if (A_STRLEN(words[i]) < c->unique_len) 
                continue;
...
static int show_prompt(struct cli_def *cli, int sockfd) 
{
    ...
    if (cli->hostname)
        len += _write_string(sockfd, cli->hostname);

    if (cli->modestring)
        len += _write_string(sockfd, cli->modestring);

    len += _write_string(sockfd, cli->promptchar);
...
int cli_loop(struct cli_def *cli, int sockfd)
{
    ...
    if (cli->telnet_protocol) {
        static const char *negotiate =
            ...
            "\xFF\xFD\x03"
            "\xFF\xFD\x01";
        _write(sockfd, negotiate, A_STRLEN(negotiate));
    }

    if ((cmd = qcom_mem_alloc(CLI_MAX_LINE_LENGTH)) == NULL) 
        return CLI_ERROR;
    ...
    while (1) {
        ...
        while (1) {
            ...
            if (cli->showprompt) {
                ...
                case STATE_LOGIN:
                    _write(sockfd, "Username: ", A_STRLEN("Username: "));
                    break;

                case STATE_PASSWORD:
                    _write(sockfd, "Password: ", A_STRLEN("Password: "));
                    break;
            ...
            sr = qcom_select(sockfd + 1, &r, NULL, NULL, &tm);
            ...
            if ((n = qcom_recv(sockfd, (char *)(&c), 1, 0)) == SOCKET_ERROR) {                             
                    ...
                    while (back--) {
                        if (l == cursor) {
                            ...
                            if (cli->state != STATE_PASSWORD && cli->state != STATE_ENABLE_PASSWORD) {
                                ...
                                _write(sockfd, "\b", 1);
                                _write(sockfd, cmd + cursor, A_STRLEN(cmd + cursor));
                                _write(sockfd, " ", 1);

                                for (i = 0; i <= (int)A_STRLEN(cmd + cursor); i++) 
                                    _write(sockfd, "\b", 1);
                            }
                        }
                        l--;
            ...
            if (c == CTRL('I')) {
                ...
                if (num_completions == 0) {
                    ...
                    l += A_STRLEN(completions[0]);
                    cmd[l++] = ' ';
                    cursor = l;

                    _write(sockfd, completions[0], A_STRLEN(completions[0]));
                    _write(sockfd, " ", 1);
                } 
                    ...
                    for (i = 0; i < num_completions; i++) {
                        _write(sockfd, completions[i], A_STRLEN(completions[i]));
            ...
            if (cursor == l) {
                ...
                if (insertmode) {
                    ...
                    for (i = l; i >= cursor; i--) cmd[i + 1] = cmd[i];
        ...
        if (cli->state == STATE_LOGIN) {
            ...
            if (cli->enable_password) {
                ...
                if (!strcmp(cli->enable_password, cmd)) 
                    allowed++;
            }
    ...
    qcom_close(cli->clientFD);
    return CLI_OK;
}


static void _print(struct cli_def *cli, 
                   int print_mode, 
                   const char *format, 
                   va_list ap)
{
    ...
    while (1) {
        ...
        if ((unsigned)n >= cli->buf_size) {
            ...
            cli->buf_size = n + 1;
            cli->buffer = qcom_mem_realloc(cli->buffer, cli->buf_size);
    ...
    do {
        ...
        if (print) {
            ...
            else if (cli->clientFD) {

                _write_string(cli->clientFD, p);
                _write_string(cli->clientFD, "\r\n");
            }
        }
    ...
    if (p && *p) {
        ...
        if (p != cli->buffer) 
            memmove(cli->buffer, p, A_STRLEN(p));
    } 
    else 
        *cli->buffer = 0;
}
...
int cli_match_filter_init(struct cli_def *cli, int argc, char **argv, struct cli_filter *filt)
{
    ...
    if (argc < 2) {
        if (cli->clientFD) 
            _write_string(cli->clientFD, "Match filter requires an argument\r\n");

        return CLI_ERROR;
    }

    filt->filter = cli_match_filter;
    filt->data = state = qcom_mem_calloc(sizeof(struct cli_match_filter_state), 1);
    ...
    if ((i = regcomp(&state->match.re, p, rflags))) {
        ...
        if (cli->clientFD) {
            _write_string(cli->clientFD, "Invalid pattern \"");
            _write_string(cli->clientFD, p);
            _write_string(cli->clientFD, "\"\r\n");
        }
...
int cli_range_filter_init(struct cli_def *cli, int argc, char **argv, struct cli_filter *filt)
{
    ...
    if (!strncmp(argv[0], "bet", 3)) { // between
        if (argc < 3) {
            if (cli->clientFD) 
                _write_string(cli->clientFD, "Between filter requires 2 arguments\r\n");
        ...
        if (argc < 2) {
            if (cli->clientFD) 
                _write_string(cli->clientFD, "Begin filter requires an argument\r\n");
    ...
    filt->data = state = qcom_mem_calloc(sizeof(struct cli_range_filter_state), 1);
...
int cli_count_filter_init(struct cli_def *cli, int argc, UNUSED(char **argv), struct cli_filter *filt)
{
    if (argc > 1) {
        if (cli->clientFD) 
            _write_string(cli->clientFD, "Count filter does not take arguments\r\n");
    ...
    if (!(filt->data = qcom_mem_calloc(sizeof(int), 1))) 
        return CLI_ERROR;
...
int cli_count_filter(struct cli_def *cli, const char *string, void *data)
{
    ...
    if (!string) { // clean up
        ...
        if (cli->clientFD) {
            sprintf(buffer, "%d\r\n", *count);
            _write_string(cli->clientFD, buffer);
        }
